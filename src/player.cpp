...
Player::Player(SDL_Renderer* renderer, int width, int height)
    : renderer(renderer), x(0), y(0), speed(5), keys{0, 0, 0, 0}, width(width), height(height) {
...
void Player::update() {
    ...
    if (keys[1] && y + 32 + speed <= height) { y += speed; }  // Move down
    if (keys[2] && x - speed >= 0) { x -= speed; }  // Move left
    if (keys[3] && x + 32 + speed <= width) { x += speed; }  // Move right
    destRect.x = x;
    destRect.y = y;
}
